<template>
  <div>
    ...
    <v-dialog width="600px" persistent v-model="dialogContact" v-if="contador == num_personas_convivencia ? false : true">
        <v-form v-model="validContact">
        <v-row  align="center" justify="center">
            <v-col cols="12" sm="8" md="6">
                <v-card>
                    <v-card-title>Registro contactos #{{contador+1}}</v-card-title>
                    <v-card-text>
                        <v-row>
                            <v-col cols="12" md="12">
                                <v-text-field
                            ...
                            <v-col cols="12" md="12">
                                ...
                                :rules="[rules.required, rules.emailRules]"></v-text-field>
                            </v-col>
                            <v-col>
                                <v-text-field
                                label="Teléfono"
                        ...
                        <v-btn color="primary" :disabled="!validContact" @click="registerFamiliar">Registrar Contacto</v-btn>
                    </v-card-text>
                </v-card>
            </v-col>
        </v-row>
...
export default {
  ...
  data() {
    return {
      ...
      valid: false,
      num_id: "",
      tiposParentescos:["Familiar","Amigo","Vecino"],
      docType: [
  ...
  methods: {
      registerFamiliar(){
          ...
          axios.post('registrarcontacto',{
              nombre: this.nombre_contacto,
              paciente_id: parseInt(this.num_id),
              parentesco: this.tiposParentescos.indexOf(this.parentesco)+1,
              ...
              this.email_contacto=""
              if(this.contador == this.num_personas_convivencia){
                  setTimeout(() => {this.$store.commit("setModuleTitle", "Pacientes");}, 1500);
              }
          }).catch(error=>{
              console.log(error)
              this.dialog = true;
      ...
      registerContact(){
          this.num_personas_convivencia < 2 ? this.num_personas_convivencia = 2 : this.num_personas_convivencia
